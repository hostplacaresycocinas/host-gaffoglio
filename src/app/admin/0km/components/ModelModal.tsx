...
export default function ModelModal({
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        listPrice: initialData.listPrice?.toString() || '',
        promotionalPrice: initialData.promotionalPrice?.toString() || '',
        currency: initialData.currency || 'ARS',
        minDownPayment: initialData.minDownPayment?.toString() || '',
        installmentsCount: initialData.installmentsCount?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Convertir campos numéricos: si está vacío, enviar null; si tiene valor, parsear
      const listPriceNum = formData.listPrice.trim()
        ? (() => {
            const parsed = parseFloat(formData.listPrice.replace(/\./g, ''));
            return isNaN(parsed) ? null : parsed;
          })()
        : null;
      const minDownPaymentNum = formData.minDownPayment.trim()
        ? (() => {
            const parsed = parseFloat(
              formData.minDownPayment.replace(/\./g, '')
            );
            return isNaN(parsed) ? null : parsed;
          })()
        : null;
      const installmentsCountNum = formData.installmentsCount.trim()
        ? (() => {
            const parsed = parseInt(formData.installmentsCount);
            return isNaN(parsed) ? null : parsed;
          })()
        : null;
      const installmentAmountNum = formData.installmentAmount.trim()
        ? (() => {
            const parsed = parseFloat(
              formData.installmentAmount.replace(/\./g, '')
            );
            return isNaN(parsed) ? null : parsed;
          })()
        : null;

      if (listPriceNum !== null && listPriceNum < 0) {
      ...
      await onSubmit(
        {
          ...
          year: currentYear,
          // Enviar string vacío en lugar de undefined para permitir borrar el campo
          versionName: initialData
            ? formData.versionName.trim()
            : formData.versionName.trim() || undefined,
          listPrice: listPriceNum !== null ? listPriceNum : undefined,
          currency: formData.currency,
          minDownPayment:
            minDownPaymentNum !== null ? minDownPaymentNum : undefined,
          installmentsCount:
            installmentsCountNum !== null ? installmentsCountNum : undefined,
          installmentAmount:
            installmentAmountNum !== null ? installmentAmountNum : undefined,
        },
          ...
          <div className='flex min-h-full items-center justify-center p-4 text-center'>
            ...
            >
              <Dialog.Panel className='w-full max-w-2xl transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all max-h-[90vh] overflow-y-auto'>
                ...
                >
                  <span>
                    {initialData ? 'Editar Modelo' : 'Agregar Modelo'}
                  </span>
                  <button
                    onClick={onClose}
                ...
                <form onSubmit={handleSubmit} className='space-y-4'>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder='Ej: Corolla, 208, Kwid...'
                      required
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={inputStyles}
                      placeholder='Ej: Active, Allure, X...'
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      rows={4}
                      placeholder='Descripción completa del modelo con características y equipamiento (soporta saltos de línea)...'
                    />
                  ...
                  <div>
                    ...
                    <div className='flex items-center space-x-1.5'>
                      ...
                      <input
                        type='text'
                        value={formatNumber(formData.listPrice)}
                        onChange={(e) => handleNumberChange(e, 'listPrice')}
                        className={inputStyles}
                        placeholder='0'
                      />
                    </div>
